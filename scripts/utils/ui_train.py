...
# trajectory plot groupbox
    def create_traj_plot_groupbox(self):
        ...
        if self.cfg.get('options', 'env_name') == 'SimpleAvoid':
            background_image_path = self.resource_path(
                'resources', 'env_maps', 'simple_world_light.png')
            ...
            background_image_path = self.resource_path(
                'resources', 'env_maps', 'NH_center.png')
            img_data = Image.open(background_image_path)
            image = np.copy(img_data)
            self.background_img = pg.ImageItem(image)
            self.traj_pw.addItem(self.background_img)
            ...
            background_image_path = self.resource_path(
                'resources', 'env_maps', 'trees_200_200.png')
            img_data = Image.open(background_image_path)
            image = np.copy(img_data)
            self.background_img = pg.ImageItem(image)
            self.traj_pw.addItem(self.background_img)
    ...
    def traj_plot_cb(self, goal, start, current_pose, trajectory_list):
        ...
        self.traj_pw.plot([current_pose[0]], [current_pose[1]], symbol='x')
        trajectory_list = np.asarray(trajectory_list)
        if trajectory_list.ndim == 2 and trajectory_list.shape[0] > 0:
            self.traj_pw.plot(
