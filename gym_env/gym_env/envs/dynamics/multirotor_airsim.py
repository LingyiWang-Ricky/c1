...
class MultirotorDynamicsAirsim():
    ...
    def reset(self):
        ...
        # reset start
        yaw_noise = self.start_angle_offset + self.start_random_angle * np.random.random()
        ...
        # take off
        # AirSim SimpleFlight can ignore moveToZAsync immediately after reset
        # unless the vehicle has explicitly taken off first.
        self.client.takeoffAsync(timeout_sec=5).join()
        self.client.moveToZAsync(-self.start_position[2], 2, timeout_sec=5).join()

        # Set the requested start pose after takeoff. Some AirSim builds apply
        # simSetVehiclePose on the next simulation tick, so advance briefly.
        ...
        self.client.simContinueForTime(max(self.dt, 0.1))
        self.client.simPause(False)

        current_z = self.get_position()[2]
        if abs(current_z - self.start_position[2]) > 0.5:
            self.client.moveToZAsync(-self.start_position[2], 2, timeout_sec=5).join()
